...
Loader::ResultStatus NCCHContainer::SetupCrypto() {
    crypto_failed = false;
    ...
    if (ncch_header.fixed_key) {
        ...
        if (!ncch_header.seed_crypto) {
            ...
            if (!opt.has_value()) {
                LOG_ERROR(Service_FS,
                          "Seed for program {:016X} not found. Import the seeddb.bin dumped from "
                          "your own console alongside your keys.",
        ...
        case 11:
            ...
            break;
        default:
            LOG_ERROR(Service_FS, "Unknown secondary key slot {}",
                      static_cast<u32>(ncch_header.secondary_key_slot));
            crypto_failed = true;
            break;
        }
    }
    ...
    if (ncch_header.version == 0 || ncch_header.version == 2) {
        ...
        // In this version, CTR for each section is the section offset prefixed by partition ID,
        // as if the entire NCCH image is encrypted using a single CTR stream.
    ...
    if (crypto_failed) {
        ...
        for (const auto& key_name : HW::AES::GetMissingNCCHKeyNames()) {
            if (!missing_list.empty()) {
                missing_list += ", ";
            }
            missing_list += key_name;
        }
        if (missing_list.empty()) {
            LOG_ERROR(Service_FS, "This title is encrypted and could not be decrypted with the "
                                  "keys that were imported.");
        } else {
            LOG_ERROR(Service_FS,
                      "This title is encrypted but these keys are missing from the imported key "
                      "file: {}",
                      missing_list);
        }
    }
...
Loader::ResultStatus NCCHContainer::LoadHeader() {
    ...
    // The NCCH header itself is never encrypted, so header fields stay readable even when the
    // keys for the content are missing. Set up the crypto here so callers that only want header
    // fields keep working; the failure is reported when the content is actually read.
    SetupCrypto();
...
Loader::ResultStatus NCCHContainer::Load() {
    ...
    if (file->IsOpen()) {
        ...
        const auto crypto_result = SetupCrypto();
        if (crypto_result != Loader::ResultStatus::Success) {
...
Loader::ResultStatus NCCHContainer::LoadSectionExeFS(const char* name, std::vector<u8>& buffer) {
    ...
    for (unsigned section_number = 0; section_number < kMaxSections; section_number++) {
        ...
        if (strcmp(section.name, name) == 0) {
            ...
            // The icon and banner are encrypted with the primary key, everything else with the
            // secondary one.
            std::array<u8, 16> key;
            ...
            if (strcmp(section.name, ".code") == 0 && is_compressed) {
                ...
                if (is_encrypted) {
                    dec.ProcessData(temp_buffer.data(), temp_buffer.data(), section.size);
                }
