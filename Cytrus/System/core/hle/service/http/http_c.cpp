...
void HTTP_C::DecryptClCertA() {
    ...
    if (file_result.Failed()) {
        // Upstream falls back to a copy of ClCertA embedded in the source. This build ships no
        // Nintendo key material of any kind, so the user's own dump is the only source.
        LOG_ERROR(Service_HTTP,
                  "ClCertA file missing. SSL services that need it will be unavailable.");
        return;
    } else {
