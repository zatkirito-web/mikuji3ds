...
private:
    /**
     * Works out whether this NCCH is encrypted and, if it is, derives the keys and counters needed
     * to decrypt it from the NCCH header and the keys the user imported. Never returns success for
     * an encrypted title whose keys are missing.
     */
    ...
    // Set when the title is encrypted but the keys needed to decrypt it are missing or unusable.
    bool crypto_failed = false;
